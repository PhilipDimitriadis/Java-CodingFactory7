...
//                throw new Exception("The amount must not be negative");
     ...
     * @param snn           the given Social Security Number
     * @throws Exception    if the SSN is not valid, or the balance is not sufficient, or the amount is negative
     */

    public void withdraw(double amount, String snn) throws SsnNotValidException, InsufficientBalanceException, NegativeAmountException {
        try {
            if (!isSsnValid(ssn)) {
